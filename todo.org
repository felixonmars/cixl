...
** finish iter
*** add iter test
* convert task to use cx_cont
* switch cx_type methods to take raw values
** replace cx_box with cx_type & void *
** add get/set methods
* add type specific allocs
** cx_type.alloc
*** add cx_type.size() method
*** use instead of cx allocs
* add tuples
** alloc whole block of memory
** use cx_box as default field type
** add Tuple<> type
** add tuple lib with get/put
** parse [] literals as tuples?
* add Concurrency/Tasks/Coros to readme
* --- cxgui
...
* ---
* add memo
** struct cx_memo/cx_memo_item
** target & cache
** add memo lib with put/deleate/clear fns
* add cont
